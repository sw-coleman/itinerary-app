...
async function sGet(key) {
  try { if (!hasStore) return memStore.has(key) ? memStore.get(key) : null;
    const r = await window.storage.get(key); return r ? JSON.parse(r.value) : null;
  } catch { return null; }
}
async function sSet(key, val) {
  try { if (!hasStore) { memStore.set(key, val); return true; }
    await window.storage.set(key, JSON.stringify(val)); return true;
  } catch { memStore.set(key, val); return false; }
}
async function sDel(key) {
  try { if (!hasStore) { memStore.delete(key); return; } await window.storage.delete(key); } catch {}
}
